...
insertScimToken
  :: (HasCallStack, MonadClient m)
  => TeamId -> ScimToken -> Maybe SAML.IdPId -> Text -> m ()
insertScimToken team token idp descr = retry x5 $ batch $ do
    setType BatchLogged
    setConsistency Quorum
    addPrepQuery ins    (team, token, idp, descr)
    addPrepQuery insRev (team, token, idp, descr)
  where
    ins, insRev :: PrepQuery W (TeamId, ScimToken, Maybe SAML.IdPId, Text) ()
    ins    = "INSERT INTO team_provisioning (team, token, idp, descr) \
             \VALUES (?, ?, ?)"
    insRev = "INSERT INTO team_provisioning_rev (team, token, idp, descr) \
             \VALUES (?, ?, ?)"
...
getScimTokens team =
  ...
  where
    ...
    sel = "SELECT token, idp, descr FROM team_provisioning WHERE team = ?"
...
deleteScimToken team token = retry x5 $ batch $ do
    ...
    addPrepQuery delRev (team, token)
  where
